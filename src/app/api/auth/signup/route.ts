...
// Importa librería para encriptar contraseñas
...
export async function POST(request: Request) {
    ...
    if (!password || password.length < 6)
        return NextResponse.json(
            ...
            {
                status: 400, // Código de estado http(bad request)
            }
        );
    ...
    try {
        ...
        if (userFound) return NextResponse.json(
            ...
            {
                status: 409 // Código HTTP 409 - conflicto de datos
            }
        );

        // 7. Encripta la contraseña con bcrypt.
